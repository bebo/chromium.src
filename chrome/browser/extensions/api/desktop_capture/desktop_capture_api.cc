...
bool DesktopCaptureListDesktopMediaFunction::RunAsync() {
  EXTENSION_FUNCTION_VALIDATE(args_->GetSize() > 0);

  std::unique_ptr<api::desktop_capture::ListDesktopMedia::Params> params =
      api::desktop_capture::ListDesktopMedia::Params::Create(*args_);
  EXTENSION_FUNCTION_VALIDATE(params.get());
  ...
  return ExecuteList(params->sources, web_contents, origin, target_name);
}

std::string DesktopCaptureListDesktopMediaFunction::GetExtensionTargetName()
    const {
  return GetCallerDisplayName();
}
...
bool DesktopCaptureRegisterDesktopMediaFunction::RunAsync() {
  EXTENSION_FUNCTION_VALIDATE(args_->GetSize() > 0);
...
std::string DesktopCaptureRegisterDesktopMediaFunction::GetExtensionTargetName()
    const {
  return GetCallerDisplayName();
}
