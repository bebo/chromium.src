...
class QsvVideoEncodeAccelerator::EncodeOutput {
 public:
...
struct QsvVideoEncodeAccelerator::BitstreamBufferRef {
...
QsvVideoEncodeAccelerator::QsvVideoEncodeAccelerator()
    : main_client_task_runner_(base::ThreadTaskRunnerHandle::Get()),
      encoder_thread_("FFMPEGQsvEncoderThread"),
      encoder_task_weak_factory_(this),
      target_bitrate_(0),
      frame_rate_(0) {
  LOG(INFO) << "Qsv Constructor. " << __func__;
}

QsvVideoEncodeAccelerator::~QsvVideoEncodeAccelerator() {}

VideoEncodeAccelerator::SupportedProfiles
QsvVideoEncodeAccelerator::GetSupportedProfiles() {
  LOG(INFO) << "Qsv Getting supported profiles.";
...
bool QsvVideoEncodeAccelerator::Initialize(VideoPixelFormat format,
                                            const gfx::Size& input_visible_size,
                                            VideoCodecProfile output_profile,
                                            uint32_t initial_bitrate,
                                            Client* client) {
  LOG(INFO) << "Initializing Qsv encoder " << __func__;
  ...
  codec_ = avcodec_find_encoder_by_name("libQsv");
  if (codec_ == NULL) {
    LOG(ERROR) << "Failed to find Qsv encoder during initialization.";
    return false;
  }
  ...
  LOG(INFO) << "Initialized Qsv encoder with codec " << "libQsv";

  implementation_name_ = "ffmpeg - libQsv";
...
void QsvVideoEncodeAccelerator::SetFrameRate(uint32_t framerate) {
...
void QsvVideoEncodeAccelerator::SetBitRate(uint32_t bitrate) {
...
void QsvVideoEncodeAccelerator::Encode(
  ...
  encoder_thread_task_runner_->PostTask(
      FROM_HERE, base::Bind(&QsvVideoEncodeAccelerator::EncodeTask,
                            encoder_task_weak_factory_.GetWeakPtr(), frame,
                            force_keyframe));
}

void QsvVideoEncodeAccelerator::DrainEncoder() {
...
void QsvVideoEncodeAccelerator::UseOutputBitstreamBuffer(
  ...
  encoder_thread_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(
          &QsvVideoEncodeAccelerator::UseOutputBitstreamBufferTask,
...
void QsvVideoEncodeAccelerator::RequestEncodingParametersChange(
    uint32_t bitrate,
    uint32_t framerate) {
  ...
  encoder_thread_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&QsvVideoEncodeAccelerator::
...
void QsvVideoEncodeAccelerator::Destroy() {
  ...
  if (encoder_thread_.IsRunning()) {
    encoder_thread_task_runner_->PostTask(
        FROM_HERE,
        base::Bind(&QsvVideoEncodeAccelerator::DestroyTask,
...
bool QsvVideoEncodeAccelerator::TryToSetupEncodeOnSeparateThread(
...
void QsvVideoEncodeAccelerator::NotifyError(
...
void QsvVideoEncodeAccelerator::EncodeTask(
...
void QsvVideoEncodeAccelerator::UseOutputBitstreamBufferTask(
    std::unique_ptr<BitstreamBufferRef> buffer_ref) {
  BVLOG(3) << __func__;
  DCHECK(encoder_thread_task_runner_->BelongsToCurrentThread());
  ...
  if (!encoder_output_queue_.empty()) {
    std::unique_ptr<QsvVideoEncodeAccelerator::EncodeOutput>
...
void QsvVideoEncodeAccelerator::ReturnBitstreamBuffer(
...
void QsvVideoEncodeAccelerator::RequestEncodingParametersChangeTask(
    uint32_t bitrate,
    uint32_t framerate) {
  BVLOG(3) << __func__;
  DCHECK(encoder_thread_task_runner_->BelongsToCurrentThread());
...
void QsvVideoEncodeAccelerator::DestroyTask() {
  BVLOG(3) << __func__;
  DCHECK(encoder_thread_task_runner_->BelongsToCurrentThread());
...
void QsvVideoEncodeAccelerator::ReleaseEncoderResources() {
  LOG(INFO) << __func__;
