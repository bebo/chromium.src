...
using directshow::kSecondsToReferenceTime;

using directshow::kMediaSubTypeI420;
using directshow::kMediaSubTypeHDYC;
using directshow::kMediaSubTypeZ16;
using directshow::kMediaSubTypeINVZ;
using directshow::kMediaSubTypeY16;
...
bool VideoSinkInputPin::IsMediaTypeValid(const AM_MEDIA_TYPE* media_type) {
...
bool VideoSinkInputPin::GetValidMediaType(int index, AM_MEDIA_TYPE* media_type) {
...
HRESULT VideoSinkInputPin::Receive(IMediaSample* sample) {
  ...
  observer_->VideoFrameReceived(buffer, length, resulting_format_, timestamp);
  return S_OK;
}
